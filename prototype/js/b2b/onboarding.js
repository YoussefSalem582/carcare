  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="flex-1 overflow-y-auto p-4 min-h-0">
      ...
      <div class="onboard-card-proto p-4">
        <div class="text-lg font-bold text-slate-900 dark:text-slate-100">Business information</div>
        <div class="text-sm text-slate-600 dark:text-slate-400 mt-0.5">Used for verification and on your public listing.</div>
        <div class="mt-4 space-y-3">
            ...
            <div><div class="label mb-1.5">Business type</div><div class="proto-input px-3.5 py-2.5 text-sm flex justify-between items-center">Independent workshop <i data-lucide="chevron-down" class="w-4 h-4 text-slate-400 dark:text-slate-500"></i></div></div>
        ...
        <div class="mt-6 flex flex-col gap-2">
          <button type="button" class="btn-ghost tap w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900">Save draft</button>
...
<div class="screen" data-screen="b2b-onboard-3">
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="flex-1 overflow-y-auto p-4 min-h-0">
        ${onboardStepper(1)}
        <div class="onboard-card-proto p-4">
          <div class="text-lg font-bold text-slate-900 dark:text-slate-100">Your service catalog</div>
          <div class="text-sm text-slate-600 dark:text-slate-400 mt-0.5">Start with presets — you can edit any time.</div>
          ...
          <div class="mt-4 space-y-2">
              ${[['Oil change (standard)','45 min','Fixed','350'],
                ...
                <div class="p-3 rounded-xl border border-slate-200 dark:border-slate-600/90 bg-gradient-to-r from-white to-teal-50/30 flex justify-between items-center gap-2 shadow-sm">
                  <div class="min-w-0">
                    <div class="font-semibold text-sm text-slate-900 dark:text-slate-100">${s}</div>
                    <div class="text-xs text-slate-600 dark:text-slate-400">${d} · <span class="badge b-slate">${t}</span></div>
                  </div>
          ...
          <div class="mt-6 flex flex-col gap-2">
            <button type="button" class="btn-ghost tap border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900" onclick="show('b2b-onboard-1')"><i data-lucide="arrow-left" class="w-4 h-4"></i> Back</button>
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="flex-1 overflow-y-auto p-4 flex items-start justify-center min-h-0">
        <div class="onboard-card-proto p-5 w-full text-center max-w-md">
          ...
          <div class="text-sm text-slate-500 dark:text-slate-400 mt-2">We’re reviewing your business and catalog. Typical turnaround is 24–48 hours.</div>
          <div class="mt-5 text-left space-y-1.5">
            ${[['Business information','done'],['Service catalog','done'],['Account checks','review'],['List on map','pending']].map(([t,s])=>`
              <div class="flex items-center gap-2 p-1.5">
                ...
                <div class="text-[10px] text-slate-500 dark:text-slate-400">${s==='done'?'Done':s==='review'?'In review':'Wait'}</div>
              </div>`).join('')}
          </div>
