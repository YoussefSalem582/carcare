  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-3 min-h-0">
        <div class="grid grid-cols-2 gap-2">
          <div class="kpi-tile kpi-tile--teal p-3 pl-4"><div class="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide">Bookings</div><div class="text-2xl font-bold text-slate-900 dark:text-slate-100">7</div><div class="text-[10px] text-emerald-600 font-semibold">+2 vs yesterday</div></div>
          <div class="kpi-tile kpi-tile--violet p-3 pl-4"><div class="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide">Revenue</div><div class="text-2xl font-bold text-slate-900 dark:text-slate-100">3.4k</div><div class="text-[10px] text-violet-600 font-medium">EGP</div></div>
          <div class="kpi-tile kpi-tile--amber p-3 pl-4"><div class="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide">Rating</div><div class="text-2xl font-bold text-slate-900 dark:text-slate-100">4.8</div><div class="text-[10px] text-amber-700">62 rev.</div></div>
          <div class="kpi-tile kpi-tile--emerald p-3 pl-4"><div class="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide">Accept</div><div class="text-2xl font-bold text-slate-900 dark:text-slate-100">96%</div><div class="text-[10px] text-emerald-600 font-semibold">OK</div></div>
        </div>
        <div class="app-panel p-3">
          <div class="flex justify-between items-center mb-2">
            <div class="font-bold text-sm">Today’s line-up</div>
            <span class="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1"><span class="dot bg-red-500"></span>1 new</span>
          </div>
          ${[['11:00','Youssef S.','Oil (std)','New','b-blue',true],['10:00','Karim H.','Brake pads','In progress','b-amber',true],['09:00','Layla M.','Oil+filter','Done','b-green',true]].map(([time,cust,svc,label,badge,act])=>`
            <div class="tap border border-slate-100 rounded-xl p-2.5 mb-2 flex justify-between items-center bg-gradient-to-r from-white to-slate-50/80 hover:border-teal-200/60" ${act?`onclick="show('b2b-booking')"`:''}>
              <div>
                ...
                <div class="text-xs text-slate-500 dark:text-slate-400">${svc}</div>
              </div>
        ...
        <div class="app-panel p-3">
          ...
          ${[['Ahmed',2,3],['Hassan',3,3],['Karim',0,3]].map(([n,b,cap])=>`
            <div class="mb-2 last:mb-0">
              <div class="flex justify-between text-xs"><span class="font-semibold">${n}</span><span class="text-slate-500 dark:text-slate-400">${b}/${cap}</span></div>
              <div class="h-1.5 rounded-full bg-slate-100 dark:bg-slate-800/80 overflow-hidden mt-0.5"><div class="h-full bg-teal-600" style="width:${(b/cap)*100}%"></div></div>
            </div>`).join('')}
        </div>
...
<div class="screen" data-screen="b2b-bookings">
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="px-3 pt-2 flex items-center justify-between">
      <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 p-0.5 flex text-xs font-semibold shadow-sm">
        <span class="px-2.5 py-1 rounded-lg bg-gradient-to-r from-teal-600 to-cyan-600 text-white shadow-sm">Week</span>
        <span class="px-2.5 py-1 rounded-lg text-slate-500 dark:text-slate-400">Day</span>
      </div>
      <div class="flex items-center gap-1">
        <button class="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 flex items-center justify-center"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
        <div class="text-xs font-semibold">Apr 13 – 19</div>
        <button class="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 flex items-center justify-center"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
      </div>
    </div>
    <div class="flex-1 overflow-y-auto p-3">
        <div class="overflow-x-auto pb-2 -mx-1">
          <div class="inline-flex gap-1 min-w-max">
            ${['S13','M14','T15','W16','T17','T18','F19'].map((d,i)=>`<div class="w-10 text-center p-1.5 rounded-lg text-xs font-semibold ${i===5?'bg-gradient-to-br from-teal-600 to-emerald-600 text-white shadow-md':'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-400'}">${d}</div>`).join('')}
          </div>
        </div>
        ...
        <div class="space-y-2">
          ${[['11:00','Youssef S.','Oil (std)','b-blue','New'],['10:00','Karim H.','Brake pads','b-amber','In progress'],['09:00','Layla M.','Oil+filter','b-green','Done']].map(([time,cust,svc,badge,l])=>`
            <div class="tap bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl p-3 flex justify-between" onclick="show('b2b-booking')">
              <div>
        ...
        <div class="mt-3 text-[10px] text-slate-500 dark:text-slate-400 flex flex-wrap gap-2">
...
<div class="screen" data-screen="b2b-booking">
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    <div class="b2b-topbar h-12 flex items-center px-3 flex-shrink-0">
      <button type="button" class="tap w-9 h-9 flex items-center justify-center -ml-1 rounded-xl hover:bg-slate-100 dark:bg-slate-800/80" onclick="show('b2b-bookings')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button>
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-3">
      <div class="app-panel p-4 ring-1 ring-indigo-500/10">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="text-xs text-slate-500 dark:text-slate-400">Sat 18 Apr · 11:00</span>
        </div>
        <div class="text-xl font-bold mt-2">Oil change (standard)</div>
        <div class="text-sm text-slate-500 dark:text-slate-400">Corolla 2019 · 82,450 km</div>
        <div class="mt-2 flex justify-between">
          <span class="text-xs text-slate-500 dark:text-slate-400">Quote</span>
        ...
        <div class="flex flex-col gap-2 mt-3">
          ...
          <button type="button" class="btn-ghost w-full text-sm border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900">Propose new time</button>
      ...
      <div class="app-panel p-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <div>
            ...
            <div class="text-[10px] text-slate-500 dark:text-slate-400">4 bookings</div>
          </div>
        </div>
      ...
      <div class="app-panel p-4">
        ...
        ${[['Ahmed',2,3,0],['Hassan',3,3,1],['Karim',0,3,2]].map(([n,b,cap,ix])=>`
          <div class="flex items-center justify-between p-2 rounded-xl border ${ix===2?'border-teal-500 bg-teal-50':'border-slate-200 dark:border-slate-600'} mb-2 last:mb-0">
            <div><div class="text-sm font-semibold">${n}</div><div class="text-xs text-slate-500 dark:text-slate-400">${b}/${cap} slots</div></div>
            ${ix===2?'<div class="w-2.5 h-2.5 rounded-full bg-teal-600"></div>':'<div class="w-4 h-4 rounded-full border-2 border-slate-200 dark:border-slate-600"></div>'}
          </div>`).join('')}
      </div>
      <div class="app-panel p-4">
        ...
        <div class="text-xs text-slate-400 dark:text-slate-500 flex justify-between py-1">Commission 10%<span>−35</span></div>
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-2">
      ...
      ${[['Oil (standard)','EGP 350','Fixed','62',true],['Brake pads','EGP 650','Fixed','22',true],['AC recharge','EGP 450','Fixed','34',true]].map(([s,p,t,b,live])=>`
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600/90 rounded-xl p-3 flex justify-between items-start shadow-sm ring-1 ring-violet-500/5">
          <div>
            <div class="text-sm font-semibold">${s}</div>
            <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5"><span class="badge b-slate">${t}</span> ${b}/mo</div>
            <div class="text-sm font-bold mt-1">${p}</div>
          </div>
          <div class="w-9 h-5 rounded-full ${live?'bg-teal-600':'bg-slate-300'}"><div class="w-4 h-4 m-0.5 rounded-full bg-white dark:bg-slate-900 float-right"></div></div>
        </div>`).join('')}
    </div>
  </div>
  ${b2bTabBar('catalog')}
...
<div class="screen" data-screen="b2b-reviews">
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    <div class="b2b-topbar h-12 flex items-center px-2 flex-shrink-0">
      <button type="button" class="tap w-9 h-9 rounded-xl hover:bg-slate-100 dark:bg-slate-800/80" onclick="show('b2b-dashboard')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button>
      <div class="font-semibold text-sm">Reviews</div>
    </div>
    <div class="flex-1 overflow-y-auto p-3 space-y-3">
      <div class="kpi-tile kpi-tile--amber p-3 pl-4">
        <div class="text-3xl font-bold text-slate-900 dark:text-slate-100">4.8 <span class="text-sm text-slate-500 dark:text-slate-400 font-normal">312 reviews</span></div>
...
<div class="screen" data-screen="b2b-payouts">
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    <div class="b2b-topbar h-12 flex items-center px-2 flex-shrink-0">
      <button type="button" class="tap w-9 h-9 rounded-xl hover:bg-slate-100 dark:bg-slate-800/80" onclick="show('b2b-more')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button>
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-3">
        <div class="kpi-tile kpi-tile--teal p-3 pl-4 text-center">
          <div class="text-xs text-slate-500 dark:text-slate-400">Available</div>
          <div class="text-2xl font-bold">EGP 12,840</div>
          <button class="btn-primary w-full text-xs py-2 mt-2">Request payout</button>
        </div>
        <div class="grid grid-cols-2 gap-2">
          <div class="kpi-tile kpi-tile--violet p-2 pl-3 text-center text-xs">Gross (mo)<div class="font-bold text-sm mt-0.5 text-slate-900 dark:text-slate-100">42,310</div></div>
          <div class="kpi-tile kpi-tile--amber p-2 pl-3 text-center text-xs">Comm. 10%<div class="font-bold text-sm mt-0.5 text-slate-900 dark:text-slate-100">4,231</div></div>
        </div>
        <div class="text-xs text-slate-500 dark:text-slate-400">Recent</div>
        ${[['11 Apr','EGP 7,578','paid'], ['4 Apr','EGP 8,892','paid'], ['28 Mar','EGP 9,486','paid']].map(([d,pay,s])=>`
          <div class="app-panel p-3 flex justify-between text-sm">
            <div><div class="font-semibold">${d}</div><div class="text-xs text-slate-500 dark:text-slate-400">Payout to bank</div></div>
...
<div class="screen" data-screen="b2b-more">
  ...
  <div class="flex-1 flex flex-col min-h-0 app-surface">
    ...
    <div class="flex-1 overflow-y-auto p-3">
      <div class="text-xs text-slate-500 dark:text-slate-400 mb-2">Account</div>
      <div class="app-panel divide-y divide-slate-100 overflow-hidden p-0 shadow-md">
        <button type="button" class="w-full p-3 flex items-center justify-between text-left tap hover:bg-amber-50/50" onclick="show('b2b-reviews')">
          ...
          <i data-lucide="chevron-right" class="w-4 h-4 text-slate-400 dark:text-slate-500"></i>
        </button>
        ...
        <div class="p-3 flex items-center justify-between text-slate-400 dark:text-slate-500">
