...
const B2C_MAP_CANVAS_OPEN = `
  <div class="flex-1 relative map-canvas map-canvas-pro overflow-hidden min-h-0">`;

const B2C_MAP_CANVAS_INTERIOR_FABS = `
...
const B2C_MAP_CANVAS_CLOSE = `
  </div>`;

/** Map search + canvas + pins/FABs only (no bottom sheet). For FTG composite backdrop. */
const B2C_MAP_HEADER_CANVAS_FABS =
  B2C_MAP_SEARCH_HEADER + B2C_MAP_CANVAS_OPEN + B2C_MAP_CANVAS_INTERIOR_FABS + B2C_MAP_CANVAS_CLOSE;

/** Full map column: header + canvas + sheet (matches legacy b2c-map body). */
const B2C_MAP_MAIN_COLUMN =
  B2C_MAP_SEARCH_HEADER +
  B2C_MAP_CANVAS_OPEN +
  B2C_MAP_CANVAS_INTERIOR_FABS +
  B2C_MAP_BOTTOM_SHEET +
  B2C_MAP_CANVAS_CLOSE;
