...
<div class="screen" data-screen="b2c-slot">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-service')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900">Pick a slot</div><div class="w-10"></div></div>
  <div class="flex-1 app-surface px-5 pt-4 overflow-y-auto min-h-0">
    <div class="flex gap-2 overflow-x-auto pb-2">
      ${['Sat 18','Sun 19','Mon 20','Tue 21','Wed 22','Thu 23','Fri 24'].map((d,i)=>`<div class="min-w-[58px] text-center p-2 rounded-xl ${i===0?'bg-gradient-to-br from-teal-600 to-cyan-600 text-white shadow-md':'border border-slate-200 bg-white'}"><div class="text-xs ${i===0?'text-white/85':'text-slate-500'}">${d.split(' ')[0]}</div><div class="font-bold">${d.split(' ')[1]}</div></div>`).join('')}
    </div>
    <div class="label mt-5 mb-2">Morning</div>
    <div class="grid grid-cols-3 gap-2">
      ${['09:00','09:30','10:00','10:30','11:00','11:30'].map((t,i)=>`<div class="text-center py-2.5 rounded-xl ${i===4?'bg-gradient-to-br from-teal-600 to-emerald-600 text-white font-semibold shadow-md':i===1||i===3?'bg-slate-100 text-slate-400 line-through':'border border-slate-200 bg-white font-medium text-sm'}">${t}</div>`).join('')}
    </div>
...
<div class="screen" data-screen="b2c-payment">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-slot')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900">Review &amp; pay</div><div class="w-10"></div></div>
  <div class="flex-1 app-surface px-5 pt-4 overflow-y-auto min-h-0">
    ...
    <div class="space-y-2">
      ...
      <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-white tap">
    ...
    <div class="mt-5 callout-info flex items-start gap-2 text-xs text-slate-600 p-3"><i data-lucide="shield-check" class="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0"></i><div>If the service can’t be completed as described, CarCare refunds within 48h. <span class="text-teal-700 font-semibold">Learn more</span></div></div>
  </div>
  <div class="cta-bar">
...
<div class="screen" data-screen="b2c-confirmed">
  ...
  <div class="cta-bar space-y-2">
    ...
    <button type="button" class="text-sm w-full text-slate-600 font-medium tap py-2" onclick="show('b2c-map')">Back to map</button>
  </div>
  <div class="home-indicator"></div>
</div>`,
...
<div class="screen" data-screen="b2c-bookings">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="px-5 pt-3 pb-3 flex items-center justify-between bg-white/90 border-b border-slate-100"><div class="font-bold text-xl tracking-tight text-slate-900">Bookings</div><button type="button" class="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-100 to-indigo-50 border border-slate-200/80 flex items-center justify-center tap"><i data-lucide="search" class="w-4 h-4 text-indigo-600"></i></button></div>
  <div class="px-5 flex gap-6 border-b border-slate-200 text-sm font-semibold bg-white">
  ...
  <div class="flex-1 overflow-y-auto px-5 pt-4 space-y-3 app-surface min-h-0">
    <div class="tap p-4 rounded-2xl bg-white border border-teal-100 shadow-md shadow-teal-900/5 ring-1 ring-teal-500/10" onclick="show('b2c-progress')">
...
<div class="screen" data-screen="b2c-progress">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-bookings')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900">Booking</div><button type="button" class="tap w-10 h-10 flex items-center justify-center rounded-xl hover:bg-slate-100"><i data-lucide="more-horizontal" class="w-5 h-5"></i></button></div>
      ...
      ${[['Booking confirmed','11:02 AM',true],['Checked in at center','11:15 AM',true],['In progress','11:22 AM','active'],['Ready for pickup','~ 12:00 PM',false],['Invoiced & paid','',false]].map(([t,s,done],i)=>`
        <div class="flex gap-3">
          <div class="flex flex-col items-center">
            <div class="w-6 h-6 rounded-full ${done===true?'bg-emerald-500':done==='active'?'timeline-dot-active':'bg-slate-200'} flex items-center justify-center">${done===true?'<i data-lucide="check" class="w-3.5 h-3.5 text-white"></i>':done==='active'?'<div class="w-2 h-2 bg-white rounded-full"></div>':''}</div>
...
<div class="screen" data-screen="b2c-review">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-bookings')"><i data-lucide="x" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900">Rate your service</div><div class="w-10"></div></div>
  <div class="flex-1 app-surface px-5 pt-4 overflow-y-auto min-h-0">
