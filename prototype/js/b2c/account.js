...
<div class="screen" data-screen="b2c-garage">
  ...
  <div class="px-5 pt-3 pb-2 flex items-center justify-between bg-white dark:bg-slate-900 border-b border-slate-100"><div class="font-bold text-xl tracking-tight text-slate-900 dark:text-slate-100">My garage</div><button type="button" class="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-600 to-cyan-600 flex items-center justify-center text-white shadow-md tap"><i data-lucide="plus" class="w-4 h-4"></i></button></div>
  <div class="flex-1 overflow-y-auto px-5 pt-4 space-y-3 app-surface min-h-0">
    ...
    <div class="tap p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600/90 shadow-sm ring-1 ring-indigo-500/5">
      <div class="flex justify-between items-start">
        <div>
          <div class="font-semibold">Hyundai Tucson</div>
          <div class="text-sm text-slate-500 dark:text-slate-400">2022 · 24,100 km</div>
        </div>
        <i data-lucide="chevron-right" class="w-5 h-5 text-slate-400 dark:text-slate-500"></i>
      </div>
    </div>
...
<div class="screen" data-screen="b2c-cardetail">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-garage')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900 dark:text-slate-100 truncate">Toyota Corolla 2019</div><button type="button" class="tap w-10 h-10 rounded-xl bg-violet-50 text-violet-700 flex items-center justify-center"><i data-lucide="pencil" class="w-5 h-5"></i></button></div>
  <div class="flex-1 overflow-y-auto px-5 pt-4 app-surface min-h-0">
    ...
    <div class="p-3 rounded-xl bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200/80 flex items-start gap-3 shadow-sm">
      ...
      <div class="flex-1"><div class="font-semibold text-sm">Oil change due in 1,200 km</div><div class="text-xs text-slate-600 dark:text-slate-400 mt-0.5">Based on your last change on 8 Feb</div></div>
    ...
    <div class="space-y-3">
      ${[['29 Mar','AC recharge','QuickFix Nasr City','EGP 450',true],
        ...
        <div class="p-3 rounded-xl border border-slate-200 dark:border-slate-600/90 bg-white dark:bg-slate-900 shadow-sm">
          <div class="flex justify-between items-start">
            <div><div class="font-semibold text-sm">${s}</div><div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">${shop}</div></div>
            <div class="text-right"><div class="font-semibold text-sm">${p}</div><div class="text-xs text-slate-500 dark:text-slate-400">${d}</div></div>
          </div>
          ${auto?'<div class="mt-2 text-[10px] uppercase tracking-wider text-teal-700 font-semibold flex items-center gap-1"><i data-lucide="zap" class="w-3 h-3"></i>Auto-logged from booking</div>':'<div class="mt-2 text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Manual entry</div>'}
        </div>`).join('')}
    </div>
    <button class="mt-4 w-full py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-sm font-semibold tap">+ Add manual entry</button>
    <div class="h-6"></div>
  </div>
  <div class="home-indicator"></div>
</div>`,
...
<div class="screen" data-screen="b2c-dashboard">
  ...
  <div class="px-5 pt-3 pb-2 flex items-center justify-between bg-gradient-to-r from-white via-teal-50/30 to-indigo-50/40 border-b border-slate-100">
    <div>
      <div class="text-sm text-slate-600 dark:text-slate-400">Good morning,</div>
      <div class="font-bold text-xl tracking-tight text-slate-900 dark:text-slate-100">Youssef</div>
    </div>
  ...
  <div class="flex-1 overflow-y-auto px-5 space-y-4 pt-3 app-surface min-h-0">
    <div class="p-4 rounded-2xl text-white bg-gradient-to-br from-orange-500 via-amber-500 to-rose-500 shadow-lg shadow-orange-500/25">
      ...
      <button class="mt-3 w-full py-2.5 rounded-xl bg-white dark:bg-slate-900/20 backdrop-blur text-sm font-semibold tap" onclick="show('b2c-reminder')">See recommendations</button>
    </div>
    ...
    <div class="app-panel p-4">
      ...
      <div class="flex items-end gap-2 mb-3">
        ...
        <div class="text-xs text-slate-500 dark:text-slate-400 pb-1">/ 100 · Good</div>
      </div>
      <div class="h-2 rounded-full bg-slate-100 dark:bg-slate-800/80 overflow-hidden ring-1 ring-slate-200/80"><div class="h-full bg-gradient-to-r from-teal-500 via-emerald-500 to-cyan-500" style="width:82%"></div></div>
    ...
    <div class="app-panel p-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1"><div class="font-semibold text-sm">Oil change · AutoPro</div><div class="text-xs text-slate-500 dark:text-slate-400">Today · 11:00</div></div>
    ...
    <div class="app-panel p-4">
      ...
      <div class="flex gap-3">
        <div class="flex-1"><div class="text-2xl font-bold">EGP 970</div><div class="text-xs text-slate-500 dark:text-slate-400">Spent</div></div>
        <div class="flex-1"><div class="text-2xl font-bold">2</div><div class="text-xs text-slate-500 dark:text-slate-400">Services</div></div>
        <div class="flex-1"><div class="text-2xl font-bold">28</div><div class="text-xs text-slate-500 dark:text-slate-400">Days streak</div></div>
      </div>
    </div>
    <div class="h-2"></div>
  </div>
...
<div class="screen" data-screen="b2c-reminder">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-dashboard')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900 dark:text-slate-100">Oil change</div><div class="w-10"></div></div>
  <div class="flex-1 overflow-y-auto px-5 pt-4 app-surface min-h-0">
    ...
    ${[['AutoPro Heliopolis','0.8 km','4.8','EGP 350','Next: 11:30'],
      ...
      <div class="tap p-3 rounded-2xl border ${i===0?'border-teal-500 bg-teal-50':'border-slate-200 dark:border-slate-600'} mb-2" onclick="show('b2c-shop')">
        <div class="flex items-center gap-3">
          <div class="w-11 h-11 rounded-xl bg-slate-200"></div>
          <div class="flex-1"><div class="flex items-center gap-1 font-semibold text-sm">${n}${i<2?'<i data-lucide="badge-check" class="w-4 h-4 text-teal-700"></i>':''}</div><div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">${d} · <i data-lucide="star" class="w-3 h-3 text-amber-500 fill-amber-500 inline -mt-0.5"></i>${r} · ${s}</div></div>
    ...
    <div class="text-xs text-center text-slate-500 dark:text-slate-400 mt-2">Or <span class="text-teal-700 font-semibold tap" onclick="show('b2c-map')">see all on map</span></div>
  </div>
  <div class="home-indicator"></div>
</div>`,
...
<div class="screen" data-screen="b2c-expenses">
  <div class="status-bar"><span>9:41</span><span></span></div>
  <div class="screen-topbar"><button type="button" class="funnel-back tap -ml-1" onclick="show('b2c-dashboard')"><i data-lucide="arrow-left" class="w-5 h-5"></i></button><div class="font-semibold text-slate-900 dark:text-slate-100">Expenses</div><span class="badge b-amber">Phase 2</span></div>
  <div class="flex-1 overflow-y-auto px-5 pt-4 app-surface min-h-0">
    ...
    <div class="grid grid-cols-2 gap-3 mt-3">
      ...
      <div class="p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900"><div class="text-xs text-slate-500 dark:text-slate-400">Other</div><div class="font-bold">EGP 0</div></div>
    </div>
    ...
    <div class="p-4 rounded-xl border border-slate-200 dark:border-slate-600">
      ...
      <div class="text-xs text-slate-500 dark:text-slate-400">Corolla · rolling 3 months</div>
      <div class="h-20 mt-2 bg-slate-50 dark:bg-slate-800/90 rounded flex items-end gap-1 p-1">
